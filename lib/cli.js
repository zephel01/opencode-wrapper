...
使い方:
  opencode-wrapper              グローバル設定に保存 (~/.config/opencode/opencode.json)
  opencode-wrapper --local      カレントディレクトリに保存 (./opencode.json)
  opencode-wrapper --global     グローバル設定に保存 (~/.config/opencode/opencode.json)
...
async function main() {
  ...
  if (options.local) {
    ...
    console.log('💾 設定ファイル: カレントディレクトリ (./opencode.json)\n');
  } else {
    // グローバル設定に保存
    config.ensureConfigDir();
    configPath = config.getConfigPath();
    console.log('💾 設定ファイル: グローバル設定 (~/.config/opencode/opencode.json)\n');
  }
